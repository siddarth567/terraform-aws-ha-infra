...
// DB connection (uses RDS endpoint from environment)
const pool = new Pool({
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    res.json({ token, user: { id: rows[0].id, name: rows[0].full_name, email: rows[0].email } });
...
app.get('/api/accounts/:id/balance', auth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT account_number, account_type, balance, currency FROM accounts WHERE id = $1 AND user_id = $2',
...
app.post('/api/transactions/transfer', auth, async (req, res) => {
  ...
  try {
    ...
    // Verify ownership & sufficient funds
    const from = await client.query('SELECT * FROM accounts WHERE id = $1 AND user_id = $2 FOR UPDATE', [from_account, req.user.id]);
    if (!from.rows[0]) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Source account not found' }); }
    if (from.rows[0].balance < amount) { await client.query('ROLLBACK'); return res.status(400).json({ error: 'Insufficient funds' }); }

    const to = await client.query('SELECT * FROM accounts WHERE id = $1 FOR UPDATE', [to_account]);
    if (!to.rows[0]) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Destination account not found' }); }
    ...
    const { rows } = await client.query(
      ...
      [from_account, to_account, amount, 'transfer', description || 'Fund transfer']
    );
...
app.get('/api/transactions', auth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT t.*, fa.account_number as from_acc_num, ta.account_number as to_acc_num 
       FROM transactions t
